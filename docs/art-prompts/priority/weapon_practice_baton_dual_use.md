# Prompt: Practice Baton — World Item + Wielded Layer

## Queue

- Status: priority
- Asset type: dual-use-item
- Target output filename: `practice_baton_world_64x64.png` and `practice_baton_wielded_layer.png`
...
```text
Use PixelLab to create a non-lethal practice baton item sprite for a cozy sci-fi life-sim RPG.

Item:
Compact training baton / stun baton without active electricity. Matte dark handle, padded gray shaft, tiny orange safety stripe. It should read as non-lethal practice equipment.
...
Restrictions:
No hand, no character body, no background, no shadow, no floor, no labels, no text, no UI frame, no blood, no gore, no aggressive effects.
```

## B) Wielded / Hand Overlay Prompt

```text
Use PixelLab to create a practice-baton-only wielded equipment layer for the attached humanoid base-body sprite sheet.

Use the attached base-body sheet as the exact alignment reference. Do not redraw the body. Draw only the baton pixels on transparent pixels.

Layer:
Compact non-lethal training baton. Matte dark handle, padded gray shaft, tiny orange safety stripe. Practical and readable, not oversized.
...
Alignment:
Place the baton in or near one hand from the attached base model in every frame. It should follow the hand position and arm swing during the walk cycle. Match the same pose, scale, center point, direction order, and row order.

Restrictions:
Only baton pixels. No body. No skin. No clothing. No hair. No face. No backpack. No attack effects. No blood. No gore. No shadow. No floor. No labels. No text. Transparent background only.
```
