...
public static class CharacterSheetLayout
{
    ...
    public static IReadOnlyList<PrototypeSpriteAnimation> EightDirectionFourRowPreviewTemplate(
        ...
        int frameSize = 64)
    {
        // The preview sheet rows are idle, walk A, walk B, and tool-ready.
        // Normal movement should never sample the tool-ready row; otherwise the
        // player appears to randomly pull out a wrench while walking. The current
        // generated sheet also lacks trustworthy back-diagonal art, so up-left and
        // up-right intentionally reuse the back-facing frames until we generate
        // true back-left/back-right poses.
        var size = new Vector2(frameSize, frameSize);
        return new[]
        {
            ...
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.IdleUpRightAnimation, 1f, new[] { DirectionFrame(origin, CharacterSheetDirection.Back, 0, size) }),
            ...
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.IdleUpLeftAnimation, 1f, new[] { DirectionFrame(origin, CharacterSheetDirection.Back, 0, size) }),
            ...
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.WalkRightAnimation, StandardWalkAnimationSpeed, SmoothDirectionWalk(origin, CharacterSheetDirection.Right, size, startRow: 1, count: 2)),
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.WalkUpRightAnimation, StandardWalkAnimationSpeed, SmoothDirectionWalk(origin, CharacterSheetDirection.Back, size, startRow: 1, count: 2)),
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.WalkUpAnimation, StandardWalkAnimationSpeed, SmoothDirectionWalk(origin, CharacterSheetDirection.Back, size, startRow: 1, count: 2)),
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.WalkUpLeftAnimation, StandardWalkAnimationSpeed, SmoothDirectionWalk(origin, CharacterSheetDirection.Back, size, startRow: 1, count: 2)),
            new PrototypeSpriteAnimation(PrototypeCharacterSprite.WalkLeftAnimation, StandardWalkAnimationSpeed, SmoothDirectionWalk(origin, CharacterSheetDirection.Left, size, startRow: 1, count: 2)),
