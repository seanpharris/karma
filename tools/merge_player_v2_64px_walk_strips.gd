...
const INPUTS := {
	"right": "res://assets/art/reference/generated/karma_player_v2_64px_right_walk_strict.jpg",
	"up_right": "res://assets/art/reference/generated/karma_player_v2_64px_up_right_walk_strict.jpg",
	"back": "res://assets/art/reference/generated/karma_player_v2_64px_back_walk_strict.jpg",
}
const CANDIDATE := "res://assets/art/sprites/generated/player_v2_engineer_8dir_4row_candidate.png"
...
func _init() -> void:
	var right := _load_strip("right")
	var up_right := _load_strip("up_right")
	var back := _load_strip("back")
	if right.is_empty() or up_right.is_empty() or back.is_empty():
		printerr("Failed to extract one or more strict walk strips.")
		quit(1)
		return
	var candidate := Image.new()
	var err := candidate.load(CANDIDATE)
	if err != OK:
		printerr("Could not load candidate ", CANDIDATE, ": ", err)
		quit(1)
		return
	candidate.convert(Image.FORMAT_RGBA8)
	_merge_walk(candidate, 2, right)
	_merge_walk(candidate, 3, up_right)
	_merge_walk(candidate, 4, back)
	_merge_walk(candidate, 5, _flip_strip(up_right))
	_merge_walk(candidate, 6, _flip_strip(right))
	_save_strip("right_walk_strict", right)
	_save_strip("up_right_walk_strict", up_right)
	_save_strip("up_left_walk_strict", _flip_strip(up_right))
	_save_strip("back_walk_strict", back)
	candidate.save_png(CANDIDATE)
	print("Merged strict 64px walk strips into ", CANDIDATE)
	quit(0)

func _merge_walk(candidate: Image, runtime_column: int, strip: Array[Image]) -> void:
	# Keep runtime row 0 from the existing sheet as idle. Rows 1-3 become clear walk steps.
	for runtime_row in range(1, 4):
		candidate.blit_rect(
			strip[runtime_row - 1],
			Rect2i(0, 0, FRAME_SIZE, FRAME_SIZE),
			Vector2i(runtime_column * FRAME_SIZE, runtime_row * FRAME_SIZE))

func _load_strip(name: String) -> Array[Image]:
	var image := Image.new()
	var err := image.load(INPUTS[name])
	if err != OK:
		printerr("Could not load ", INPUTS[name], ": ", err)
		return []
	image.convert(Image.FORMAT_RGBA8)
	var cell_w := image.get_width() / SOURCE_COLUMNS
	var frames: Array[Image] = []
	for col in SOURCE_COLUMNS:
		var source_rect := Rect2i(col * cell_w, 0, cell_w, image.get_height())
		frames.append(_normalize_cell(image.get_region(source_rect)))
	return frames
...
func _flip_strip(strip: Array[Image]) -> Array[Image]:
	var flipped: Array[Image] = []
	for frame in strip:
...
func _save_strip(name: String, frames: Array[Image]) -> void:
	var sheet := Image.create(FRAME_SIZE * SOURCE_COLUMNS, FRAME_SIZE, false, Image.FORMAT_RGBA8)
	sheet.fill(Color(0, 0, 0, 0))
	for col in SOURCE_COLUMNS:
		sheet.blit_rect(frames[col], Rect2i(0, 0, FRAME_SIZE, FRAME_SIZE), Vector2i(col * FRAME_SIZE, 0))
	sheet.save_png(OUTPUT_DIR + "/player_v2_64px_%s_extract.png" % name)
